...
from player import HoldOption, TurnState
from settings import BARREL, CUBES, EDGES, STEPS, WIN


class Game:
    def __init__(self, player_set, rng=None, collect_history=True):
        """Конструктор игры."""
    ...
    @staticmethod
    def cube_count(cube_set):
        """Подсчёт одинаковых кубиков."""

        return dict(Counter(cube_set))

    @staticmethod
    def grade(cube_count):
        """Подсчёт количества очков для выбранных кубов."""
    ...
    @staticmethod
    def is_scoring_face(face, count):
        """Проверяет, участвует ли значение в очковой комбинации."""
    ...
    @classmethod
    def hold_options(cls, cube_count):
        """Возвращает все способы отложить кубы с очками."""
    ...
    @staticmethod
    def check_strike(cube_count):
        """Определяет небитку по полному броску."""
    ...
    def build_turn_state(self, player, turn_score, remaining_cubes, rolls_left):
        frozen_score = player.barrel_score if player.barrel_score is not None else player.total_score
        return TurnState(
    ...
    def apply_bank(self, player, turn_score):
        """Записывает очки игроку с учётом бочки."""
        ...
        if player.total_score >= BARREL and player.total_score < WIN:
            player.barrel_score = player.total_score

        return player.total_score >= WIN

    def play_turn(self, player):
        """Разыгрывает полный ход игрока."""
